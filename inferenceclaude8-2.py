...
sample_idx = 7  # test sample index for inference
run_optimization = False  # whether to run the optimization step after initial inference

# --- Face/corner initialization ---
# 'harmonic'           : solves a global 2D discrete Laplace equation matching the Dirichlet boundaries
...
interior_init_mode = (
    "nearest_match"  # 'harmonic', 'nearest_match', 'zeros', 'constant', 'random'
)
...
J_left_int = load_model(
    "flux_internal8_left",
    input_dim=in_dim,
    output_dim=y_J_left.shape[1],
    hidden_dims=(64, 128, 64, 32),
)
J_right_int = load_model(
    "flux_internal8_right",
    input_dim=in_dim,
    output_dim=y_J_right.shape[1],
    hidden_dims=(64, 128, 64, 32),
)
J_top_int = load_model(
    "flux_internal8_top",
    input_dim=in_dim,
    output_dim=y_J_top.shape[1],
    hidden_dims=(64, 128, 64, 32),
)
J_bottom_int = load_model(
    "flux_internal8_bottom",
    input_dim=in_dim,
...
J_left_bnd = load_model(
    "flux_boundary8_left",
    input_dim=in_dim,
    output_dim=y_J_left.shape[1],
    hidden_dims=(64, 128, 64, 32),
)
J_right_bnd = load_model(
    "flux_boundary8_right",
    input_dim=in_dim,
    output_dim=y_J_right.shape[1],
    hidden_dims=(64, 128, 64, 32),
)
J_top_bnd = load_model(
    "flux_boundary8_top",
    input_dim=in_dim,
    output_dim=y_J_top.shape[1],
    hidden_dims=(64, 128, 64, 32),
)
J_bottom_bnd = load_model(
    "flux_boundary8_bottom",
    input_dim=in_dim,
    output_dim=y_J_bottom.shape[1],
    hidden_dims=(64, 128, 64, 32),
)
...
unscale_dict_int = {
    "left": (
        torch.tensor(
            load_scalers("flux_internal8_left")[1].scale_,
            dtype=torch.float32,
            device=device,
        ),
        torch.tensor(
            load_scalers("flux_internal8_left")[1].mean_,
            dtype=torch.float32,
            device=device,
        ),
    ),
    "right": (
        torch.tensor(
            load_scalers("flux_internal8_right")[1].scale_,
            dtype=torch.float32,
            device=device,
        ),
        torch.tensor(
            load_scalers("flux_internal8_right")[1].mean_,
            dtype=torch.float32,
            device=device,
        ),
    ),
    "top": (
        torch.tensor(
            load_scalers("flux_internal8_top")[1].scale_,
            dtype=torch.float32,
            device=device,
        ),
        torch.tensor(
            load_scalers("flux_internal8_top")[1].mean_,
            dtype=torch.float32,
            device=device,
        ),
    ),
    "bottom": (
        torch.tensor(
            load_scalers("flux_internal8_bottom")[1].scale_,
            dtype=torch.float32,
            device=device,
        ),
        torch.tensor(
            load_scalers("flux_internal8_bottom")[1].mean_,
            dtype=torch.float32,
            device=device,
        ),
    ),
...
unscale_dict_bnd = {
    "left": (
        torch.tensor(
            load_scalers("flux_boundary8_left")[1].scale_,
            dtype=torch.float32,
            device=device,
        ),
        torch.tensor(
            load_scalers("flux_boundary8_left")[1].mean_,
            dtype=torch.float32,
            device=device,
        ),
    ),
    "right": (
        torch.tensor(
            load_scalers("flux_boundary8_right")[1].scale_,
            dtype=torch.float32,
            device=device,
        ),
        torch.tensor(
            load_scalers("flux_boundary8_right")[1].mean_,
            dtype=torch.float32,
            device=device,
        ),
    ),
    "top": (
        torch.tensor(
            load_scalers("flux_boundary8_top")[1].scale_,
            dtype=torch.float32,
            device=device,
        ),
        torch.tensor(
            load_scalers("flux_boundary8_top")[1].mean_,
            dtype=torch.float32,
            device=device,
        ),
    ),
    "bottom": (
        torch.tensor(
            load_scalers("flux_boundary8_bottom")[1].scale_,
            dtype=torch.float32,
            device=device,
        ),
        torch.tensor(
            load_scalers("flux_boundary8_bottom")[1].mean_,
            dtype=torch.float32,
            device=device,
        ),
    ),
...
# 3c. 2D Harmonic Solver Initialization
def compute_harmonic_initialization():
  print(" -> Solving 2D discrete Laplace potential on interface skeleton...")
  nx_nodes = n_subx + 1
  ny_nodes = n_suby + 1
  total_nodes = nx_nodes * ny_nodes

  A = sp.lil_matrix((total_nodes, total_nodes), dtype=np.float64)
  rhs = np.zeros(total_nodes, dtype=np.float64)

  true_corners_np = true_global_corners.cpu().numpy().ravel()
  bnd_corner_uids_np = global_corner_indices.cpu().numpy().ravel()

  corner_val_dict = {}
  mean_corner_scalar = float(np.ravel(U_corners_bnd_mean)[0])
  for uid, val in zip(bnd_corner_uids_np, true_corners_np):
    corner_val_dict[int(uid)] = float(val) + mean_corner_scalar

  for iy in range(ny_nodes):
    for ix in range(nx_nodes):
      idx = iy * nx_nodes + ix
      if ix == 0 or ix == nx_nodes - 1 or iy == 0 or iy == ny_nodes - 1:
        A[idx, idx] = 1.0
        rhs[idx] = corner_val_dict.get(idx, 0.0)
      else:
        A[idx, idx] = -4.0
        A[idx, idx - 1] = 1.0
        A[idx, idx + 1] = 1.0
        A[idx, idx - nx_nodes] = 1.0
        A[idx, idx + nx_nodes] = 1.0

  u_grid = spla.spsolve(A.tocsr(), rhs).reshape(ny_nodes, nx_nodes)

  harm_face_vals = np.zeros((n_faces, 3), dtype=np.float32)
  harm_corner_vals = np.zeros((n_corners, 1), dtype=np.float32)

  face_n_pts = U_face_int_basis.shape[0]
  mean_corner_int = float(np.ravel(U_corners_int_mean)[0])

  for iy in range(ny_nodes):
    for ix in range(nx_nodes):
      uid = iy * nx_nodes + ix
      harm_corner_vals[uid, 0] = float(u_grid[iy, ix]) - mean_corner_int
  ...
  for _, row in face_lookup_df.iterrows():
    ...
    if direction == "bottom":
      v0, v1 = u_grid[ny, nx], u_grid[ny, nx + 1]
    elif direction == "top":
      v0, v1 = u_grid[ny + 1, nx], u_grid[ny + 1, nx + 1]
    elif direction == "left":
      v0, v1 = u_grid[ny, nx], u_grid[ny + 1, nx]
    elif direction == "right":
      v0, v1 = u_grid[ny, nx + 1], u_grid[ny + 1, nx + 1]

    edge_profile = np.linspace(v0, v1, face_n_pts, dtype=np.float32).reshape(
        1, -1
    )
    modes = project_to_rom(edge_profile, U_face_int_basis, U_face_int_mean)
    harm_face_vals[uid, :3] = modes[0, :3]

  return harm_face_vals, harm_corner_vals


init_face_values_harm, init_corner_values_harm = (
    compute_harmonic_initialization()
)

# 3d. Final Mode Selection
print(f"--- FACE/CORNER INIT MODE: '{interior_init_mode}' ---")
if interior_init_mode == "harmonic":
  init_face_values = init_face_values_harm
  init_corner_values = init_corner_values_harm
elif interior_init_mode == "nearest_match":
...
# =========================================================================
# 4. OPTIMIZATION CORE (ROUTED & UNSCALED FLUX CONSERVATION)
# =========================================================================
...
def compute_fluxes_latent():
  """Aligned with compute_fluxes_routed: unscales flux outputs back to standard

  ROM latent coordinate scale and dynamically routes across elements before
  calculating horizontal and vertical interface conservation residuals.
  """
  ...
  # Evaluate and unscale fluxes (matching inference8 routing)
  flux_left_int = (
      J_left_int(X_int_scaled) * unscale_dict_int["left"][0]
  ) + unscale_dict_int["left"][1]
  flux_right_int = (
      J_right_int(X_int_scaled) * unscale_dict_int["right"][0]
  ) + unscale_dict_int["right"][1]
  flux_top_int = (
      J_top_int(X_int_scaled) * unscale_dict_int["top"][0]
  ) + unscale_dict_int["top"][1]
  flux_bottom_int = (
      J_bottom_int(X_int_scaled) * unscale_dict_int["bottom"][0]
  ) + unscale_dict_int["bottom"][1]

  flux_left_bnd = (
      J_left_bnd(X_bnd_scaled) * unscale_dict_bnd["left"][0]
  ) + unscale_dict_bnd["left"][1]
  flux_right_bnd = (
      J_right_bnd(X_bnd_scaled) * unscale_dict_bnd["right"][0]
  ) + unscale_dict_bnd["right"][1]
  flux_top_bnd = (
      J_top_bnd(X_bnd_scaled) * unscale_dict_bnd["top"][0]
  ) + unscale_dict_bnd["top"][1]
  flux_bottom_bnd = (
      J_bottom_bnd(X_bnd_scaled) * unscale_dict_bnd["bottom"][0]
  ) + unscale_dict_bnd["bottom"][1]

  num_elements = X_f_const.shape[0]

  flux_left = torch.empty(
      (num_elements, flux_left_int.shape[1]),
      dtype=torch.float32,
      device=device,
  )
  ...
  flux_right = torch.empty(
      (num_elements, flux_right_int.shape[1]),
      dtype=torch.float32,
      device=device,
  )
  ...
  flux_top = torch.empty(
      (num_elements, flux_top_int.shape[1]),
      dtype=torch.float32,
      device=device,
  )
  ...
  flux_bottom = torch.empty(
      (num_elements, flux_bottom_int.shape[1]),
      dtype=torch.float32,
      device=device,
  )
...
# =========================================================================
# 7. ROM MODES DIAGNOSTICS & HEATMAP (LIKE FILE 8)
# =========================================================================
...
plt.tight_layout()
plt.savefig(f"Plots/Inference8_2Result_7_{interior_init_mode}.pdf", dpi=300)
plt.show()
...
def extract_features_and_targets(df: pd.DataFrame, operator: str, flux_direction: str = None, domain_type: str = 'internal'):
...
def load_dataset_split(domain_type: str, operator: str, flux_direction: str = None):
    ...
    X_train, y_train, input_dim, output_dim = extract_features_and_targets(df_train, operator, flux_direction, domain_type)
    X_val, y_val, _, _ = extract_features_and_targets(df_val, operator, flux_direction, domain_type)
    X_test, y_test, _, _ = extract_features_and_targets(df_test, operator, flux_direction, domain_type)
...
def main():
    ...
    save_path = os.path.join(plot_dir, "operator_training8modified_summary.pdf")

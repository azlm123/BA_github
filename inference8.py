...
interior_init_mode = (
    "nearest_match"  # 'per_subdomain_match', 'nearest_match', 'zeros', 'constant', 'random'
)
...
def load_combined_data_stacked(
    ...
    # 1. Subdomain-level mask (for solution reconstruction)
    is_sub_bnd_mask = stacked_df["is_boundary_domain"].values
    ...
    return X_f, X_bnd, y, stacked_df, is_sub_bnd_mask
...
def reconstruct_full_solution(
    ...
    """Uses the subdomain mask to project back through internal vs boundary POD bases."""
    if hasattr(is_sub_bnd_mask, "cpu"):
        is_sub_bnd_mask = is_sub_bnd_mask.cpu().numpy()

    is_bnd_mask = is_sub_bnd_mask.astype(bool)
...
x_f, x_bnd, y_S, sample_df, is_sub_bnd_mask = load_combined_data_stacked(
...
# MASK 1: Subdomain Boundary Mask (for Solution operator and physical field reconstruction)
sub_bnd_mask_t = torch.tensor(is_sub_bnd_mask, dtype=torch.bool, device=device)
sub_int_mask_t = ~sub_bnd_mask_t

# MASK 2: Face Boundary Masks (for Flux computation & routing)
face_bnd_masks = {
    "bottom": torch.tensor((sample_df["U_face_bottom_is_bnd"] == 1).values, dtype=torch.bool, device=device),
    "right": torch.tensor((sample_df["U_face_right_is_bnd"] == 1).values, dtype=torch.bool, device=device),
    "top": torch.tensor((sample_df["U_face_top_is_bnd"] == 1).values, dtype=torch.bool, device=device),
    "left": torch.tensor((sample_df["U_face_left_is_bnd"] == 1).values, dtype=torch.bool, device=device),
}
...
# =========================================================================
# 2. MESH TOPOLOGY & FIXED GLOBAL BOUNDARIES
# =========================================================================
...
is_global_corner_mask = (
    ((corner_lookup_df["ny_index"] == 0) & (corner_lookup_df["corner_position"].isin(["bottom_left", "bottom_right"])))
    | ((corner_lookup_df["ny_index"] == n_suby - 1) & (corner_lookup_df["corner_position"].isin(["top_left", "top_right"])))
    | ((corner_lookup_df["nx_index"] == 0) & (corner_lookup_df["corner_position"].isin(["bottom_left", "top_left"])))
    | ((corner_lookup_df["nx_index"] == n_subx - 1) & (corner_lookup_df["corner_position"].isin(["bottom_right", "top_right"])))
)
...
for _, row in global_corners_df.iterrows():
    nx, ny, pos = row["nx_index"], row["ny_index"], row["corner_position"]
...
# =========================================================================
# 3. INITIALIZATION (GLOBAL NEAREST & PER-SUBDOMAIN MATCHING)
# =========================================================================
df_train_int = pd.read_csv(csv_internal_train_path).rename(
    columns=lambda x: x.replace("F_sub_1_", "F_sub_").replace("U_sub_1_", "U_sub_")
)
df_train_bnd = pd.read_csv(csv_boundary_train_path).rename(
...
print(f" -> Best matching global train sample ID: {best_sample_id} | Rel F_sub Err: {min_rel_err:.4e}")
...
for _, row in face_lookup_df.iterrows():
    nx, ny, direction, uid = int(row["nx_index"]), int(row["ny_index"]), row["direction"], int(row["n_unique"])
    sub_row = best_df[(best_df["nx_index"] == nx) & (best_df["ny_index"] == ny)]
    for i in range(3):
        init_face_values_nearest[uid, i] = sub_row[f"U_face_{direction}_mode_{i}"].values[0]

for _, row in corner_lookup_df.iterrows():
    nx, ny, pos, uid = int(row["nx_index"]), int(row["ny_index"]), row["corner_position"], int(row["n_unique"])
    sub_row = best_df[(best_df["nx_index"] == nx) & (best_df["ny_index"] == ny)]
    init_corner_values_nearest[uid, 0] = sub_row[f"U_corners_{pos}_mode_0"].values[0]

# -------------------------------------------------------------------------
# 3b. Per-Subdomain Match (Location Locked: nearest F_sub at each (nx, ny))
# -------------------------------------------------------------------------
...
for i, row in sample_df.iterrows():
    ...
    q = query_F_all[i]
    loc_pool = df_train[(df_train["nx_index"] == nx) & (df_train["ny_index"] == ny)].reset_index(drop=True)
...
for _, row in corner_lookup_df.iterrows():
    nx, ny, pos, uid = int(row["nx_index"]), int(row["ny_index"]), row["corner_position"], int(row["n_unique"])
    sub_row = persub_match_lookup[(nx, ny)]
    init_corner_values_persub[uid, 0] = sub_row[f"U_corners_{pos}_mode_0"]
...
if interior_init_mode == "nearest_match":
    ...
    init_face_values = (
        interior_init_random_mean + interior_init_random_std * torch.randn((n_faces, 3))
    ).numpy().astype(np.float32)
    init_corner_values = (
        interior_init_random_mean + interior_init_random_std * torch.randn((n_corners, 1))
    ).numpy().astype(np.float32)
else:
    raise ValueError(f"Unknown interior_init_mode: {interior_init_mode!r}")

# -------------------------------------------------------------------------
# 3d. Structurally Pin Boundary DOFs
# -------------------------------------------------------------------------
...
# =========================================================================
# 4. FLUX COMPUTATION (USING FACE-LEVEL BOUNDARY MASKS)
# =========================================================================
...
def evaluate_directional_flux(direction: str, X_full: torch.Tensor):
    """Evaluates flux using the face-level boundary mask for the specific direction."""
    is_face_bnd = face_bnd_masks[direction]
    is_face_int = ~is_face_bnd

    out = torch.empty((X_full.shape[0], y_J_left.shape[1]), dtype=torch.float32, device=device)

    # 1. Evaluate internal faces with internal flux operator
    if torch.any(is_face_int):
        X_in = X_full[is_face_int]
        mean_in, scale_in = flux_x_scalers_int[direction]
        X_scaled = (X_in - mean_in) / scale_in
        pred_scaled = J_int[direction](X_scaled)
        scale_y, mean_y = unscale_dict_int[direction]
        out[is_face_int] = pred_scaled * scale_y + mean_y

    # 2. Evaluate global boundary faces with boundary flux operator
    if torch.any(is_face_bnd):
        X_bd = X_full[is_face_bnd]
        mean_bd, scale_bd = flux_x_scalers_bnd[direction]
        X_scaled = (X_bd - mean_bd) / scale_bd
        pred_scaled = J_bnd[direction](X_scaled)
        scale_y, mean_y = unscale_dict_bnd[direction]
        out[is_face_bnd] = pred_scaled * scale_y + mean_y
...
def compute_fluxes_latent():
    ...
    flux_left = evaluate_directional_flux("left", X_full)
    flux_right = evaluate_directional_flux("right", X_full)
    flux_top = evaluate_directional_flux("top", X_full)
    flux_bottom = evaluate_directional_flux("bottom", X_full)
...
def scipy_objective_latent(x_vec):
    ...
    with torch.no_grad():
        face_values_interior.copy_(
            torch.tensor(x_vec[:n_face_dofs].reshape(-1, 3), dtype=torch.float32, device=device)
        )
        corner_values_interior.copy_(
            torch.tensor(x_vec[n_face_dofs:].reshape(-1, 1), dtype=torch.float32, device=device)
        )
    ...
    g_face = face_values_interior.grad.detach().cpu().numpy().ravel().astype(np.float64)
    g_corner = corner_values_interior.grad.detach().cpu().numpy().ravel().astype(np.float64)
...
if run_optimization:
    ...
    res = minimize(
        ...
        method="L-BFGS-B",
        jac=True,
        options={"maxiter": 400, "maxls": 50, "disp": True, "gtol": 1e-8, "ftol": 1e-14},
    )
    ...
    with torch.no_grad():
        face_values_interior.copy_(
            torch.tensor(res.x[:n_face_dofs].reshape(-1, 3), dtype=torch.float32, device=device)
        )
        corner_values_interior.copy_(
            torch.tensor(res.x[n_face_dofs:].reshape(-1, 1), dtype=torch.float32, device=device)
        )
else:
...
# =========================================================================
# 6. SOLUTION RECONSTRUCTION (USING SUBDOMAIN BOUNDARY MASK)
# =========================================================================
...
with torch.no_grad():
    ...
    # Evaluate internal subdomains via S_int
    X_int_final = torch.cat(
        [
            X_f_const[sub_int_mask_t],
            f_bot[sub_int_mask_t],
            f_rig[sub_int_mask_t],
            f_top[sub_int_mask_t],
            f_lef[sub_int_mask_t],
            c_bl[sub_int_mask_t],
            c_br[sub_int_mask_t],
            c_tr[sub_int_mask_t],
            c_tl[sub_int_mask_t],
            X_bnd_const[sub_int_mask_t],
        ],
        dim=1,
    )
    ...
    # Evaluate boundary subdomains via S_bnd
    X_bnd_final = torch.cat(
        [
            X_f_const[sub_bnd_mask_t],
            f_bot[sub_bnd_mask_t],
            f_rig[sub_bnd_mask_t],
            f_top[sub_bnd_mask_t],
            f_lef[sub_bnd_mask_t],
            c_bl[sub_bnd_mask_t],
            c_br[sub_bnd_mask_t],
            c_tr[sub_bnd_mask_t],
            c_tl[sub_bnd_mask_t],
            X_bnd_const[sub_bnd_mask_t],
        ],
        dim=1,
    )
    ...
    predicted_rom_modes[sub_int_mask_t.cpu().numpy()] = sol_int_modes
    predicted_rom_modes[sub_bnd_mask_t.cpu().numpy()] = sol_bnd_modes

    target_rom_modes = y_S

# =========================================================================
# 7. FIELD RECONSTRUCTION & ERROR VISUALIZATION
# =========================================================================
U_pred_full = reconstruct_full_solution(
    predicted_rom_modes, is_sub_bnd_mask, rom_data, n_subx=n_subx, n_suby=n_suby
)
U_true_full = reconstruct_full_solution(
    target_rom_modes, is_sub_bnd_mask, rom_data, n_subx=n_subx, n_suby=n_suby
)
...
if is_nearest_match:
    ...
    # Calculate error between reconstructed prediction and nearest training seed

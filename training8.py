...
def extract_features_and_targets(df: pd.DataFrame, operator: str, flux_direction: str = None):
    """
    Extract features and targets without applying face-level boundary filters,
    ensuring all faces of a given subdomain train together.
    """
    # 1. Feature columns
...
def load_dataset_split(domain_type: str, operator: str, flux_direction: str = None):
    """
    Load data splits based purely on the domain type (subdomain-level).
    - internal: strictly dataset_operator_internal_*.csv
    - boundary: strictly dataset_operator_boundary_*.csv
    """
    def read_split(split):
        # Directly read the corresponding dataset without concatenating internal/boundary data
...
def train_single_operator(domain_type: str, operator: str, flux_direction: str = None, 
    ...
    model_name = f"{operator}_{domain_type}_train8"+("_hyperpara" if USE_HYPERPARAMS_CSV else "") + (f"_{flux_direction}" if flux_direction else "")
    print("\n" + "=" * 80)
...
def main():
    ...
    plt.tight_layout()
    save_path = os.path.join(plot_dir, f"operator_train8"+("_hyperpara" if USE_HYPERPARAMS_CSV else "")+"_summary.pdf")

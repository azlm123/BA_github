...
from training_f_separate import device, relative_error
...
def get_feature_blocks(df: pd.DataFrame, domain_type: str = 'internal'):
    ...
    # Only count boundary-flag columns toward the U-block index range if they
    # are actually part of feature_cols (i.e. domain_type == 'boundary').
    # Otherwise X has no columns for them and U_idx would run past X's width.
    n_f, n_u, n_v = len(f_e_cols), len(u_f_cols), len(u_v_cols)
    n_b = len(bnd_flag_cols)

    F_idx = list(range(0, n_f))
    U_idx = list(range(n_f, n_f + n_u + n_v + n_b))  # U_face + U_corners + (boundary flags if present)
...
def main():
    ...
    def run_both(domain, operator, flux_direction=None):
        model_name = f"{operator}_{domain}8modified" + (f"_{flux_direction}" if flux_direction else "")
        print("\n" + "=" * 80)
